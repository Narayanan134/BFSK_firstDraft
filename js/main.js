import { BID, OE, INVM,PM1,PM2,AD, ROUTPUT,I1,I2,PM3,PM4,DD1,DD2,MUX,phi1,phi2,phi3,phi4 } from './Block.js'
...
import { drawOnOffWave } from './onOffWaveGraph.js';
...
function setup_modulation() {
    ...
    myblocks.set('sampler', new OE(546.6-79, 132.5, 200, 100));
    myblocks.set('prodmod1', new PM1(940.32-79, 52.5, 220, 100));
    myblocks.set('phi1', new phi1(940.32-79, 12.5, 100, 35));
    myblocks.set('invertermod', new INVM(840.32-79, 242.5, 220, 100));
    myblocks.set('prodmod2', new PM2(1100.32-79, 242.5, 220, 100));
    myblocks.set('phi2', new phi2(1100.32-79, 348.5, 100, 35));
...
function setup_demodulation() {
    ...
    myblocks.set('phi3', new phi3(546.6-79, 638.5, 100, 35));
    myblocks.set('prodmod4', new PM4(546.6-79, 792.5, 200, 100));
    myblocks.set('phi4', new phi4(546.6-79, 898.5, 100, 35));
...
function openModal(obj, dblClick = false) {
    ...
    $(modalName).on('shown.bs.modal', function () {
        if (modalName === '#sourceWaveGraph') {
            drawSourceWave();
        } else if (modalName === '#onOffWaveGraph') {
            drawOnOffWave();
            ...
            drawEncodedWave();
        } else if (modalName == '#phi1WaveGraph') {
            drawProd1Wave();
        } else if (modalName == '#phi2WaveGraph') {
            drawProd2Wave();
        } else if (modalName == '#adderWaveGraph') {
            drawAdderWave();
        }
    });
