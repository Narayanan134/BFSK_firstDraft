let sourceWaveCanvas = document.getElementById('adderWaveCanvas');
...
const vertical_scale_element = document.getElementById("adder_vertical_scale_factor");
const horizontal_scale_element = document.getElementById("adder_horizontal_scale_factor");
const f1=document.getElementById("ipfrequency1");
const f2=document.getElementById("ipfrequency2");
...
function plotSine(ctx, amplitude, frequency, xOffset, yOffset, vertical_scaling_factor, horizontal_scaling_factor,f1,f2) {
    var width = 1000;
    ...
    for(let i=0;i<=8;i++){
        if(main_signal[i]==1){
            ...
            t.forEach((val) => {
                x.push(Math.sqrt(2/(1/50)) * Math.cos(2 * Math.PI * f2 * val));
            });
        }
    }
...
export function drawAdderWave() {
    ...
    plotSine(sourceWaveCtx, wave_amplitude, wave_frequency, orgx, mid_of_line, vertical_scaling_factor, horizontal_scaling_factor,f1_wave,f2_wave);
    requestAnimationFrame(drawAdderWave);
}